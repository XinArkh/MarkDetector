...
class detector:
    ...
    # mode=0，摄像头模式，name和number是要保存文件的文件名
    # mode！=0，本地模式，name和number不仅是要保存的文件名，也是所选本地视频的名字
    def __init__(self, mode=0, name='video', number=1):
        self.name = name
        self.number = number
        self.fullname = self.name + str(self.number) + '.avi'
        # while os.path.exists(self.fullname):
        #     self.number += 1
        #     self.fullname = self.name + str(self.number) + '.avi'
        if mode == 0:
    ...
    def saveVideo(self, fps=20):
        ...
        # 保存原始视频
        self.videoWriter1 = cv2.VideoWriter(
    ...
    es = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 4))  # 膨胀操作用到的蒙版
    def _rect(self, area):
    ...
    def _save(self, centers):
        if self._videoFlag:
            self.videoWriter1.write(self.frame)
            self.videoWriter2.write(self.frameCopy)
    ...
    def run(self, threshold=0.9, bgPath=None, area=40000):
        ...
        while success and cv2.waitKey(1) == -1:
            ...
            self.res = cv2.matchTemplate(
                self.blurredFrame,
                self.blurredTemplate,
