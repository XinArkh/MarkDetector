...
clear
close all
%1.txt文件中的是惯性传感器的数据第一列是惯性传感器自身进行卡尔曼滤波之后的数据
%第2列是传感器的角速度数据；第三列是传感器只根据三轴加速度得到的参考重力方向的角度
%第四列是系统时钟
...
plot(t1(200:2315),Y(200:2315),'LineWidth',2,'Color','red')
hold on
plot(t12(200:2315),a(777:2892,1),'Color',[1 0.8 0],'LineWidth',2)
ylabel('angle(°)','FontName','Palatino Linotype');
xlabel({'t(s)','(b) From IMU Individual Kalman Filter'},'FontName','Palatino Linotype');
axis([2 15 -80 100])
...
plot(t1(200:2315),Y(200:2315),'LineWidth',2,'Color','red')
hold on
plot(t13(200:2315),0.99*b22(200:2315),'Color',[0.1 0.9 0.8],'LineWidth',2)
ylabel('angle(°)','FontName','Palatino Linotype');
xlabel({'t(s)','(c) From Visual Identity'},'FontName','Palatino Linotype');
axis([2 15 -80 100])
